...
Reuses GHL_SQL_SERVER / GHL_SQL_DATABASE (same VM + shared dcr_warehouse as the
rest of the project) but authenticates with the read-only MCP_SQL_* credentials
(the `dcr_ro` login, shared with GHL_API) so the LLM can never mutate data even
if a guardrail has a bug.
"""
