...
def resolve_users(user: str) -> list[tuple[str, str]]:
    """Resolve `user` (a numeric UserId or a name substring) to [(UserId, Name)]."""
    u = (user or "").strip()
    if not u:
        return []
    if u.lstrip("-").isdigit():
...
def default_window(since: str | None, until: str | None) -> tuple[str, str]:
    """Default to the last 7 days (UTC) when since/until are omitted. `until` is
    EXCLUSIVE. Both validated as ISO dates."""
    import datetime as _d

    today = _d.datetime.now(_d.timezone.utc).date()
    s = since or (today - _d.timedelta(days=7)).isoformat()
    u = until or (today + _d.timedelta(days=1)).isoformat()
    return check_date(s, "since"), check_date(u, "until")
